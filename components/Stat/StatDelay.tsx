...
export function DelayTooltip({
  ...
  return (
    <>
      <dt>
        <Result>攻撃待機時間</Result>{parent.equal}
        <small>
          {f.fixedDelay ? <>
            <Level level={fdCol}>固定値</Level>
          </> : <>
            基礎値
            {delayMul !== 100 && <>{parent.multiply}
            ...
            {formationBuff !== 100 && <>{parent.multiply}
              <Level level={formationBuff < 100}>編成バフ</Level></>}
            {beastFormationBuff !== 100 && <>{parent.multiply}
      ...
      <dd>
        <Result b>{f.result}{parent.frame}</Result>{parent.equal}
        {f.fixedDelay ? <>
          <Level level={fdCol}>{f.fixedDelay}{parent.frame}</Level>
        </> : <>
          {f.delay}{parent.frame}
          {delayMul !== 100 && <>{parent.multiply}
          ...
          {formationBuff !== 100 && <>{parent.multiply}
            <Level level={formationBuff < 100}>{formationBuff + parent.percent}</Level></>}
          {beastFormationBuff !== 100 && <>{parent.multiply}
