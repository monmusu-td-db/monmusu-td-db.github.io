...
export function AttackSpeedTooltip({
  ...
  return (
    <>
      <dt>
        <Result>攻撃動作速度</Result>{parent.equal}
        <small>
          {f.fixedAttackSpeed !== undefined ? <>基礎値</> : (
            <>
              {b && parent.bStart}
              基礎値
              {p && (
          ...
          {!!f.attackMotionMul && <>{parent.multiply}
            <Level level={ammCol}>攻撃動作倍率</Level></>}
          {attackSpeedBuff !== 100 && <>{parent.divide}
            <Level level={asbCol}>攻撃速度バフ倍率</Level></>}
        </small>
      </dt>
      <dd>
        <Result b>{f.attackSpeedResult}{parent.frame}</Result>{parent.equal}
        {f.fixedAttackSpeed !== undefined ? (
          <>
            {f.fixedAttackSpeed}{parent.frame}
          </>
        ) : (
          <>
            {b && parent.bStart}
            {f.attackSpeedBase}{parent.frame}
            {p && (
              <>
                {parent.minus}
                <Positive>{f.attackSpeedPotential}{parent.frame}</Positive>
              </>)}
            {b && parent.bEnd}
          </>
        )}
