...
>
  ...
  public getFormationTooltip(setting: Setting): ReactNode {
    ...
    const weaponBaseBuffAdd = ssMulEnabled && !!f.weaponBaseBuff
      ? Math.abs(f.weaponBaseBuff - 100) : f.weaponBaseBuff;
        ...
        <T.Equation>
          {d => (
            <>
              ...
              <T.Expression>
                <T.Brackets
                  enabled={(!!f.baseBuff || ssMulEnabled || !!f.weaponBaseBuff) && !!f.potential}
                >
                  {d ? "基礎値" : f.base}
                ...
                <T.Multiply enabled={ssMulEnabled || !!f.weaponBaseBuff}>
                  <T.Brackets enabled={ssMulEnabled && !!f.weaponBaseBuff}>
                    {ssMulEnabled && (
                      <T.Info>
                        {d ? "サブスキル乗算倍率" : f.subskillMul + sign.PERCENT}
                      </T.Info>
                    )}
                ...
                <T.Plus enabled={!!f.weaponUpgrade}>
                  <T.Positive>
                    {d ? "武器強化" : f.weaponUpgrade}
                  </T.Positive>
                </T.Plus>
                <T.Plus enabled={!!f.subskillAdd}>
                  <T.Info>
                    {d ? "サブスキル加算値" : f.subskillAdd}
                  </T.Info>
                </T.Plus>
              </T.Expression>
            </>
