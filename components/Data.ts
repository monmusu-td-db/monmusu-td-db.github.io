...
export class Percent {
  ...
  static sum(...values: (number | undefined)[]) {
    return values.map(v => v === undefined ? 100 : v).reduce((a, c) => a + c - 100, 100);
  }
...
export const Condition = {
  ...
  of(...lists: (readonly Condition[] | undefined)[]): Condition[] {
    const ret: Condition[] = [];
    lists.forEach(list => list?.forEach(kvp => {
...
export const StaticDamage = {
  ...
  baseStatTypeOf(key: StaticDamageKey): BaseStatType | undefined {
    switch (key) {
      case staticDamage.HP_BASE: return stat.hp;
      case staticDamage.ATTACK_BASE: return stat.attack;
      case staticDamage.DEFENSE_BASE: return stat.defense;
      case staticDamage.RESIST_BASE: return stat.resist;
    }
  },
  ...
  textOf(key: StaticDamageKey): string {
    switch (key) {
      ...
      case staticDamage.RESIST_BASE:
        return "配置前" + StatType.nameOf(StaticDamage.baseStatTypeOf(key) as BaseStatType);
...
export type AttackDebuff = {
  readonly key: typeof staticDamage.ATTACK_BASE | typeof AttackDebuff.enemyAttack
  readonly value: number
}
...
export const Debuff = {
  calculate(
    ...
    if ("valueMul" in obj) {
      ...
      return obj.valueAdd * Accumulation.calculate({ time: obj.time, attackSpeed, interval });
    }
  }
} as const;
...
export const Round = {
  average<T extends Rounds | undefined>(rounds: T): number | Extract<T, undefined> {
    if (rounds === undefined)
...
export const JsonRound = {
  parse<T extends JsonRound | undefined>(value: T): Rounds | Extract<T, undefined> {
...
export const JsonSkill = {
  parse(value: JsonSkill): Skill {
    ...
    const duration = value.duration === Duration.single ?
      Duration.single : JsonDuration.parse(value.duration);
    const ret: SkillBase = {
      ...v,
      duration,
...
function parseSkill(skill: Partial<SkillBase>, jsonSkill: Partial<JsonSkill>): void {
...
const JsonSkillFeature = {
  parse(value: readonly JsonSkillFeature[] | undefined): readonly SkillFeature[] | undefined {
...
export interface FormationBuff {
  readonly targets: ReadonlySet<typeof FormationBuff.all | Element | BaseClassName | Species>
  ...
  readonly potentialBonus?: Omit<FormationBuff, "targets" | "require" | "potentialBonus">
}
...
export type JsonFormationBuff = JsonFormationBuffDiff & Omit<FormationBuff, keyof JsonFormationBuffDiff>
export const JsonFormationBuff = {
  parse(value: readonly JsonFormationBuff[]): FormationBuff[] {
    return value.map(v => {
      ...
      return {
        ...v,
        targets: new Set(v.targets.map(t => {
...
export type FormationBuffRequire = typeof formationBuffRequire[keyof typeof formationBuffRequire]
...
export type TableData<TData, TColumn extends keyof TData> = Readonly<{
  ...
  comparer: (setting: Setting, key: TColumn, target: TData) => string | number | undefined,
...
type ClassEquipmentName = typeof classEquipmentName[keyof typeof classEquipmentName]
...
export const ClassName = {
  ...
  keyOf(value: ClassName | undefined): ClassNameKey | undefined {
    if (value === undefined) return;
    return Object.entries(className).find(kvp => kvp[1] === value)?.[0] as ClassNameKey | undefined;
  },

  equipmentNameOf(value: ClassName | undefined): ClassEquipmentName | undefined {
...
export const Element = {
  ...
  parseElements(arr: readonly string[] | undefined): ReadonlySet<Element> | undefined {
  ...
  keyOf(value: Element): ElementKey {
    return Object.entries(element).find(v => v[1] === value)?.[0] as ElementKey;
  },
...
export const Species = {
  ...
  keyOf(value: unknown): SpeciesKey | undefined {
    return Object.entries(species).find(v => v[1] === value)?.[0] as SpeciesKey | undefined;
  },
} as const;
...
export type Target = number | typeof target.self | typeof target.all | typeof target.other
  | typeof target.hit | number[]
...
export const Target = {
  ...
  getBlockTarget(value: TargetBase | undefined, block: number | undefined): Target | undefined {
...
export const MoveType = {
  ...
  keyOf(value: MoveType): MoveTypeKey {
    return Object.entries(moveType).find(v => v[1] === value)?.[0] as MoveTypeKey;
  },
...
export const DamageType = {
  ...
  keyOf(value: DamageType): DamageTypeKey {
    return Object.entries(damageType).find(v => v[1] === value)?.[0] as DamageTypeKey;
  },
...
const placementEntries = Object.entries(placement) as [PlacementKey, typeof placement[PlacementKey]][];
...
export const JsonPotentials = {
  parse(value: readonly string[] = []): readonly (Potential | undefined)[] {
    return value.slice(0, 5).map(p => potentialList.find(v => v.name === p)?.name);
  },

  toSet(src: Readonly<JsonPotentials> = []): Set<Potential> {
    const ret = src.filter(str => Potential.list.find(v => v.name === str) !== undefined);
...
export const Potential = {
  ...
  getEffectValue(statType: StatType, potentials: readonly (Potential | undefined)[]): number {
    return potentials.map(p => {
      return potentialList.find(src => src.stat === statType && src.name === p)?.value ?? 0;
    }).reduce((a: number, c) => a + c, 0);
  },

  filter(statType: StatType, potentials: readonly (Potential | undefined)[]): ReadonlySet<Potential> {
    const ret = new Set<Potential>();
    potentials.forEach(p => {
      if (p !== undefined
        && potentialList.findIndex(src => src.stat === statType && src.name === p) !== -1
      ) ret.add(p);
    });
    return ret;
  }
...
export interface IntervalBaseFactors extends AttackSpeedInBattleFactors, DelayFactors {
